...
function Sidebar({
  ...
  return (
    <div
      ...
      data-side={side}
      data-slot="sidebar"
      style={{ overflow: "hidden" }}  // Prevent overflow from breaking rounded corners
    >
      ...
      <div
        className={cn(
          ...
          side === "left"
            ? "left-2 top-2 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]"
      ...
      >
        <div
          ...
          style={{ borderRadius: "12px", overflow: "hidden" }}  // Ensures the border-radius is applied and content doesn't overflow
        >
          {children}
        </div>
      </div>
  ...
  const button = (
    <Comp
      ...
      onClick={(event: React.MouseEvent<HTMLElement>) => {
        onClick?.(event)
        if (closeOnSelectMobile && isMobile && !event.defaultPrevented) {
          setOpenMobile(false)
        }
      }}
      {...props}
